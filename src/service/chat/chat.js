...
import { collection, doc, docData, collectionData } from 'rxfire/firestore';
...
const ChatRoomMemberRefPath = (ShipmentKey, ChatRoomKey) =>
  FirebaseApp.firestore()
    .collection('Shipment')
    .doc(ShipmentKey)
    .collection('ChatRoom')
...
const ChatRoomMessageReaderRefPath = (ShipmentKey, ChatRoomKey, ProfileKey) =>
  FirebaseApp.firestore()
    .collection('Shipment')
    .doc(ShipmentKey)
    .collection('ChatRoom')
...
export const GetChatRoomList = (ShipmentKey, UserKey) =>
  collection(ShipmentRefPath(ShipmentKey).where('ChatRoomMemberList', 'array-contains', UserKey));
...
export const AddChatRoomMember = (ShipmentKey, ChatRoomKey, Data) =>
  from(ChatRoomMemberRefPath(ShipmentKey, ChatRoomKey).add(Data));

export const UpdateChatRoomMember = (ShipmentKey, ChatRoomKey, ChatRoomMemberKey, Data) =>
  from(
...
export const DeleteChatRoomMember = (ShipmentKey, ChatRoomKey, ChatRoomMemberKey) =>
  from(
    ChatRoomMemberRefPath(ShipmentKey, ChatRoomKey)
      .doc(ChatRoomMemberKey)
      .delete()
...
export const GetChatRoomMemberList = (ShipmentKey, ChatRoomKey) =>
  collection(ChatRoomMemberRefPath(ShipmentKey, ChatRoomKey));

export const UpdateChatRoomMessageReader = (ShipmentKey, ChatRoomKey, ProfileKey, Data) =>
  from(
    ChatRoomMessageReaderRefPath(ShipmentKey, ChatRoomKey, ProfileKey).set(Data, { merge: true })
  );

export const LeaveChatRoomMember = (ShipmentKey, ChatRoomKey, ChatRoomMemberKey) =>
  from(
    ChatRoomMemberRefPath(ShipmentKey, ChatRoomKey)
      .doc(ChatRoomMemberKey)
...
export const isChatRoomMember = (ShipmentKey, ChatRoomKey, UserKey) =>
  collection(
    ChatRoomMemberRefPath(ShipmentKey, ChatRoomKey).where('ChatRoomMemberUserKey', '==', UserKey)
  ).pipe(filter(data => data.length > 0));

export const isInternalChatRoom = (ShipmentKey, ChatRoomKey) =>
  docData(ChatRoomRefPath(ShipmentKey, ChatRoomKey), 'ChatRoomKey').pipe(
    map(ChatRoomDoc => ChatRoomDoc.data().ChatRoomIsInternal)
  );

export const GetCompanyInternalChatRoom = (ShipmentKey, CompanyKey) =>
  collectionData(
